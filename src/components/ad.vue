  ...
  <div>
    <div class="flex-between top-box">
      <div class="flex-start">
        ...
        <div v-on:click="Go('http://bizsubmitportal.tripln.com/')" class="system-title">沈阳市行权治理(监督)一体化平台</div>
      </div>
      <div class="">
          ...
          <div v-if="show == 1" class="container1">
            ...
            <div id="container1-area-id" class="container1-area">
              ...
              <div id="link6" class="container1-card">
                <Tabs type="card">
                  <TabPane label="办件公示">
                    <Table :border="true" size="small" :data="publicitycontent" :columns="publicityTitle"><template slot-scope="{ row }" slot="action"><Button size="small" v-on:click="viewPublicity(row)" style="margin-right: 5px" type="primary">查看公示内容</Button></template></Table>
                    <Modal title="公示具体内容" :mask-closable="false" :footer-hide="true" v-model="modalPublic">
                      <Card :dis-hover="true">
                        <Form :model="formItemPublic" :label-width="120">
                          <FormItem label="公示标题:">
                            {{formItemPublic.title}}
                          </FormItem>
                          <FormItem label="公示时间:">
                            {{formItemPublic.start_time}}
                          </FormItem>
                          <FormItem label="公示结束时间:">
                            {{formItemPublic.end_time}}
                          </FormItem>
                          <FormItem label="公示人:">
                            {{formItemPublic.person}}
                          </FormItem>
                          <FormItem label="公示内容:">
                            {{formItemPublic.content}}
                          </FormItem>
                        </Form>
                      </Card>
                    </Modal>
...
export default {
  ...
  methods: {
    ...
    viewPublicity (row) {
      console.log(row, '公示内容')
      this.formItemPublic = row.public_params
    ...
    initGuide () {
      const that = this
      // this.$scp.postback('http://10.40.3.156:32381/api/uc/bizplatform/flowSummaryServiceImpl/querySummaryByFlowInfoId', {
      this.$scp.postback('http://bizsubmitportal.tripln.com/api/ac/bizsubmitportal/matterService/querySummaryByDefKey', {
        id: this.processKey
        // 测试用
        ...
        if (data) {
          ...
          // 权利类型
          if (data.matter_type) {
            if (data.matter_type === '0') {
              data.matter_type = '普通'
            }
            if (data.matter_type === '1') {
              data.matter_type = '惠民'
            }
            if (data.matter_type === '2') {
              data.matter_type = '惠企'
            }
          }
          if (data.power_type) {
    ...
    click (e) {
      ...
      if (e !== 1) {
        ...
        const path = '/generalreporting/generalReport/lmq-home-page/' + this.processKey + '/' + this.flowId
        this.$router.push({
          path: path
        })
        this.$router.push(location)
